...
`/clear` 是 Claude Code 用户级 slash 命令，**脚本和子进程无法直接调用**。因此用以下机制代替：

- 当前 `claude -p` 子会话完成阶段并保存快照后**直接退出**
- 外部 `autonomous-runner.js` 启动**新的 `claude -p` 子会话**
- 新子会话通过 `SessionStart` hook 自动加载 `latest_state.json`
- 新子会话读取 `autonomous-state.json` 确认自主模式仍为 ON
- 新子会话从快照中读取 `stage.next`（或 `next_action`），按新选题独立推进

**为什么清理**（目标不变，实现方式改为新子会话）：
...
- 每个选题尽量控制在独立短上下文内完成
- 进入新选题前必须结束当前 `claude -p` 子会话、启动新子会话，不是可选优化
- 大选题（>3 天）必须拆分成子选题，每个子选题完成后保存快照 + 切换新子会话
- `SessionStart` hook 已配置自动加载快照，确保新子会话不会丢失进度
