...
# 📚 文档同步规则（Doc Sync · 8 文档版）

> **作用**：里程碑或增量完成后，强制同步根目录下 8 个核心文档 + `CHANGELOG.md`，**避免用户文档出现"功能已交付但文档未提"的状态漂移**。
> **触发节点**：每个里程碑（M1~Mn）/ 增量（A~G / A+B+C+D 子模块）/ 大版本发版（vX.Y.0）/ 路线图重写。
> **创建日期**：2026-06-25（v1 4 文档）｜ **2026-06-26 强化（v2 6 文档）**：补 01.md 必同步 + npm script + 用户可见判断标准 ｜ **2026-06-27 强化（v3 8 文档）**：补 PROJECT-CONTEXT.md + README.md 必同步
> **背景**：
...
> - **2026-06-27**：PROJECT-CONTEXT.md 停留在 v1.9 / README.md 停留在 v2.0.0，但工程已 v3.0.5（M25~M32），**新会话启动加载的还是过期信息**。同类问题不能再发生。
...
| # | 文档 | 路径 | 同步内容 | 同步时机 |
|:--|:-----|:-----|:---------|:--------|
| 1 | **README** | `README.md` | GitHub 仓库首页展示：1 句定位 + 安装 + 测试 | **🔴 大** 必做（新用户第一印象）|
| 2 | **1 分钟全貌** | `PROJECT-CONTEXT.md` | session-init 自动加载：核心系统表 + 11 命令 + L5 进度 | **🔴 大** 必做（每次启动都看）|
| 3 | **用户速查主表** | `01_AI-ClaudeCode-最佳实践精简.md` | §三 速查表新增命令/能力行、§二 行为约定新增条款 | **🔴 大** 必做（用户最常看的入口） |
| 4 | **工作空间说明** | `02_工作空间功能介绍.md` | §2.X 新增功能章节、§现状速览表追加 | **🔴 大** 必做（功能字典） |
| 5 | **演进路线** | `04_自我演进路线.md` | 0.4 节增量状态、第十二章里程碑表、顶部"升级时间" | **🔴 大** 必做 |
| 6 | **版本计划** | `03_版本迭代计划.md` | 第四节 P0/P1 完成状态、第七节发版流程记录 | **🔴 大** 必做 |
| 7 | **CLAUDE.md** | `CLAUDE.md` | 工作空间结构树、核心能力表、规则文件清单、快速操作 | **🔴 大** 必做 |
| 8 | **CHANGELOG.md** | `CHANGELOG.md` | **每次必写**（放最顶部的 Unreleased 段） | **🟡 小** 也必做 |
| 9 | `package.json` | `package.json` | version 字段 | **🏁 里程碑** + 发版 |

> 原则：
> - **`CHANGELOG.md` 是源事实**（代码已完成→就写条目）
> - **`04.md` / `03.md` 是状态汇总** → 反向同步 CHANGELOG
> - **`CLAUDE.md` / `01.md` / `02.md` / `PROJECT-CONTEXT.md` / `README.md` 是用户可见层** → **🔴 大** 必做
...
```bash
# 提交后立即同步（8 文件并行）
# 1. README.md - 1 句定位 + 安装 + 测试
# 2. PROJECT-CONTEXT.md - 核心系统表 + 11 命令 + L5 进度
# 3. 01.md - §三 速查表 + §二 行为约定（如新增命令/能力）
# 4. 02.md - §2.X 新功能章节 + §现状速览表
# 5. 04.md - §0.4 增量段 + §十二 里程碑表
# 6. 03.md - 第四节 P0/P1 状态
# 7. CLAUDE.md - 工作空间结构 / 核心能力表 / 快速操作 / 规则清单
# 8. CHANGELOG.md - 顶部 Unreleased 段
```
...
```bash
# 9 文件全同步（含 package.json）
# 1-8. 同节点 1
# 9. package.json - version 字段
```

### 节点 4：路线图重写

```bash
# 8 文件全部同步，CLAUDE.md 顶部"启动协议"段也要重审
```
...
### 🚨 改 CLAUDE.md 的硬红线（2026-06-29 借鉴 khazix/neat-freak）

| 红线 | 数值 | 超出怎么办 |
|:-----|:-----|:-----------|
| **净涨幅** | **≤ 30 行 / 次同步** | 写历史叙事而非补规则 → 回头删 / 迁 docs |
| **总量上限** | **~300 行 / ~15KB**（软）| 先精简顶部 blockquote + 历史叙事段 |
| **禁止写法** | "X 起 Y 上线，详见 docs/Z.md" blockquote 段 | 删 / 迁 CHANGELOG / 迁 04.md |

**判据**：**"下次 AI 写代码时没看到这条，会不会犯错？"** 会 → 进；不会 → 删/迁 docs/进 memory。

**执行顺序**（必跑）：
1. **先精简**（破除膨胀）→ 心态是"什么不该在这"
2. **再补漏**（本次增量）→ 心态是"什么该补到这"
3. 两件事不能合并 —— 混着做两头不到位
...
> 8. **M54 起**：扫 01.md / 04.md / 03.md / PROJECT-CONTEXT.md / README.md 的版本号 metadata 是否与 `package.json` 一致
> 9. 输出 anomaly 报告
...
- `scripts/orchestrator/test-doc-sync.js` — 自动化测试（验证 6 文档日期一致性 + 01/02 必含 M_N 段 + **版本号 metadata 与 package.json 一致性**）
- `scripts/orchestrator/sync-roadmap.js` — 自动同步 04.md 顶部元数据 + 版本号 metadata 到 01/04/03/PROJECT-CONTEXT/README
...
- [x] **v3 强化（2026-07-01 M54）**：版本号 metadata 自动同步 + test-doc-sync.js 一致性断言
- [ ] 集成到 proactive-scan.js 7 维度（增量 C 扩展）
- [x] **写测试 `test-doc-sync.js`（验证 6 文档日期一致性 + 01/02 必含 M_N 段）** — M24.6
- [x] **test-doc-sync.js 加版本号一致性检查（01/04/03/PROJECT-CONTEXT/README）** — M54
- [x] **加 npm script `npm run doc:check`（手动检查漂移）** — M24.6
- [x] **self-discipline 决策树补"文档同步"分支（6 文档自检清单）** — M24.6
- [x] **sync-roadmap.js 扩展版本号 metadata 同步（--no-version 跳过）** — M54
