...
> **v1.1.0 · 4 阶段自动化 · 阶段级验证 · 失败立即停止 · 支持 dry-run / skip / only**
...
| 阶段 | 默认命令 | 阶段验证标准 | 跳过建议 |
|:-----|:---------|:-------------|:---------|
| **test** | `npm test --silent` | 退出码 0，无失败用例 | ❌ 不建议（流水线入口） |
| **simplify** | （预留，当前无独立脚本）| 简化后测试仍通过，无新增风格问题 | ✅ 可安全跳过 |
| **review** | （预留，当前无独立脚本）| 审查通过，无高风险问题未处理 | ✅ 可安全跳过 |
| **commit** | `git commit --allow-empty -m '...'` | commit 成功创建 | ❌ 不建议 |
...
每个阶段都回答：**"我怎么知道这个阶段完成了？"**

| 弱指令 | 强验证 |
|:-------|:-------|
| "跑测试" | "npm test 退出码 0，无失败用例" |
| "简化代码" | "简化后测试仍通过，无新增 lint 问题" |
| "提交" | "git commit 成功创建" |
