...
description: >
  ...
  关键决策写入快照不询问、安全时自动 commit。支持 single / always / toggle / off 四种模式。
tags:
  - autonomous-mode
  - state-machine
  - snapshot-driven
...
> **v1.0 · single / always / toggle / off · 状态机驱动 · 快照保护**
...
| 文件 | 作用 |
|:-----|:-----|
| `scripts/orchestrator/autonomous.js` | 开关引擎（single/always/toggle/off）|
| `scripts/orchestrator/autonomous-runner.js` | 循环执行器（spawn `claude -p` 子会话）|
| `scripts/orchestrator/test-autonomous.js` | 测试（62/62 通过）|
