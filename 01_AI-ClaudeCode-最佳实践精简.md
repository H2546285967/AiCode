...
| 搜索知识 | `recall 关键词` | 左脑搜索（默认 Node TF-IDF 语义，`--grep` 强制 grep） |
| 查看系统 | `/status` | 看监控 |
...
| **MEMORY.md 体检** | `npm run memory:health` | 4 项硬约束（200/25KB + 单条 100 行 + 体量倒挂），session-init Step 1 自动显示 compact 结果（v3.0.6 M48-D + v3.0.8 M62，16/16 测试）|
...
### 📌 复杂任务用 [plan] 块（先评估后执行）（M54 兑现）

- **触发**：≥3 文件 / ≥2 模块 / 改根配置 / 用户说"先评估"
- **格式**：见 [`.claude/rules/plan-protocol.md`](../.claude/rules/plan-protocol.md)
- **三命令**：
  - `/ok` — 批准 AI 的 plan
  - `/no` — 取消
  - `/plan-execute` — 执行已批准的 plan（派 `claude -p` 子会话）
- **简单任务**：单文件 / 单模块 / ≤2 行 → 直接干，不用 plan
...
**作用**：快照替代 next 队列中的待执行项，新会话自动发现"该继续做 XXX"。
...
**什么时候用**：开始执行一个核心功能，希望 AI 自主决策直到完成。

**行为**：AI 自主完成当前功能 → 自动 commit → 自动保存快照 → 自动关闭。
...
**什么时候用**：你要离开很长一段时间，让 AI 持续执行 next 队列任务。
