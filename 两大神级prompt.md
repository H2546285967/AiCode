...
| 第一性原理 vs 比较思维 对比表 |                                                  |                                        |
| ----------------------------- | ------------------------------------------------ | -------------------------------------- |
| 维度                          | 第一性原理（拆解重构）                           | 比较思维（类比模仿）                   |
| 思维路径                      | 回归事物最底层的本质事实，向上推演构建新方案。   | 观察他人的做法，通过微调或模仿来改进。 |
| 思考结果                      | 往往能实现颠覆性创新（如SpaceX火箭的成本优化）。 | 容易陷入思维定势，局限于行业既有规则。 |
...
已做完并上线。核心是两件事:
1.让 Path B 回滚边界变诚实:
S scripts/ops/deploy.sh现在会写,deploy/current-deploy-*元数据，记录本轮 path、commit、baseDB dump 路径。
$scripts/ops/deploy-rollback.sh 如果发现失败部署是 Path B，会明确提示:代码已闪回，但 DB 不自动pg_restore ，需要人工判断restore 还是 forward-fix。
2.让回测闸门更准:
$ scripts/ops/preflight-deploy.sh 不再因为文档里出现 scoring/translate 这类词误触发回测门，只按真实会改变线上产出的runtime/data/lib 代码路径触发。
对抗式审查里实际抓到并修了两个坑:目录匹配漏了:会导致真正的modules/feed/runtime/enrich/scoring.ts 不触发;PathBdump元数据如果由root写，下一次 svc_aihot可能覆盖失败。都已修掉。
